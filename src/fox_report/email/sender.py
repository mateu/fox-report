...
class EmailSender:
    ...
    def __init__(self, config: dict):
        ...
        try:
            self.jinja_env = Environment(
                ...
                autoescape=self.format_type == "html",
            )
    ...
    def _send_via_smtp_with_images(
        self, subject: str, body: str, report: dict, json_report_path: str | None = None
    ) -> tuple[bool, str, str]:
        """
        ...
        try:
            ...
            for i, match in enumerate(matches):
                ...
                image_cids.append(
                    (cid, match.group(2))
                )  # group(2) is now the base64 data
                # Replace just the data URL with CID reference, keeping anchor tags intact
                html_body = html_body.replace(
                    f"data:image/jpeg;base64,{match.group(2)}", f"cid:{cid}"
                )
            ...
            text_part = MIMEText(
                "Please view this email in HTML format to see the fox detection report with images.",
                "plain",
            )
            ...
            error_msg = f"SMTP send failed: {e!s}"
            logger.error(error_msg)
            return False, "", error_msg

    def _send_via_smtp(
        self, subject: str, body: str, json_report_path: str | None = None
    ) -> tuple[bool, str, str]:
        """
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    "Full email message written to debug file: %s", debug_file_path
                )
            except Exception as e:
            ...
            try:
                ...
                if self.smtp_config.get("use_tls", True):
                    ...
                    logger.info(
                        "TLS started - Response code: %s, Message: %s",
                        tls_response[0],
                        tls_response[1].decode("utf-8") if tls_response[1] else "None",
                    )
                else:
                ...
                try:
                    ...
                    if send_result:
                        ...
                        logger.error(
                            "Failed to send to recipients: %s",
                            ", ".join(failed_recipients),
                        )
                        raise EmailSendError(
                            f"Failed to send to: {', '.join(failed_recipients)}"
                        )
                    else:
                        ...
                        try:
                            with open(
                                debug_file_path, "a", encoding="utf-8"
                            ) as debug_file:
                                debug_file.write("\n=== SEND RESULT ===\n")
                            ...
                            logger.warning(
                                "Failed to update debug file with send result: %s",
                                str(e),
                            )

                except smtplib.SMTPRecipientsRefused as recipients_error:
                    logger.error(
                        "All recipients refused - Error: %s",
                        recipients_error.recipients,
                    )
                    raise
                except smtplib.SMTPSenderRefused as sender_error:
                    logger.error(
                        "Sender refused - Code: %s, Message: %s, Sender: %s",
                        sender_error.smtp_code,
                        sender_error.smtp_error.decode("utf-8")
                        if sender_error.smtp_error
                        else "None",
                        sender_error.sender,
                    )
                    raise
                ...
                if server:
                    try:
                        ...
                        logger.warning(
                            "Error closing SMTP connection: %s", str(close_error)
                        )
    ...
    def _send_via_system_mail(
        ...
        try:
            ...
            if json_report_path and os.path.exists(json_report_path):
                ...
                cmd = [mail_cmd, "-s", subject, "-a", json_report_path, self.recipient]
            else:
                # Send without attachment
                cmd = [mail_cmd, "-s", subject, self.recipient]
            ...
            if result.returncode == 0:
                success_msg = (
                    f"Email sent successfully via system mail to {self.recipient}"
                )
                logger.info(success_msg)
                return True, result.stdout, result.stderr
            else:
                error_msg = (
                    f"Mail command failed (exit {result.returncode}): {result.stderr}"
                )
                logger.error(error_msg)
            ...
            error_msg = f"Failed to execute mail command: {e!s}"
            logger.error(error_msg)
            return False, "", error_msg
            ...
            if isinstance(report, dict):
                ...
                if isinstance(totals, dict) and "total_events" in totals:
                    ...
                    if isinstance(events_by_camera, dict):
                        total = sum(
                            (
                                cam_data.get("stats", {}).get(
                                    "event_count", len(cam_data.get("events", []))
                                )
    ...
    def _render_html_body(self, report: dict, markdown_content: str) -> str:
        ...
        try:
            ...
            logger.warning(
                "Failed to generate HTML with thumbnails, falling back to basic HTML: %s",
                str(e),
            )
    ...
    def send_email(
        self, report: dict, markdown_content: str, json_report_path: str | None = None
    ) -> tuple[bool, str, str]:
        """
        ...
        try:
            ...
            error_msg = f"Failed to render email body: {e!s}"
            logger.error(error_msg)
            return False, "", error_msg
        ...
        if self.use_smtp:
            ...
            if smtp_valid:
                ...
                if (
                    ...
                    and report.get("events_by_camera")
                ):
                    success, stdout, stderr = self._send_via_smtp_with_images(
                        subject, email_body, report, json_report_path
                    )
                else:
                    success, stdout, stderr = self._send_via_smtp(
                        subject, email_body, json_report_path
                    )
                if success:
                    return success, stdout, stderr
                else:
                    logger.warning(
                        "SMTP send failed, falling back to system mail: %s", stderr
                    )
            else:
