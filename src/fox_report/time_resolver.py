...
class TimeResolver:
    ...
    def _load_config(self, config_file: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise ValueError(f"Error parsing configuration file: {e!s}") from e
    ...
    def _calculate_astral_times(self, target_date: date) -> tuple[datetime, datetime]:
        ...
        self.logger.debug(
            "Using coordinates: lat=%s, lon=%s, elevation=%s",
            latitude,
            longitude,
            elevation,
        )
        ...
        self.logger.debug(
            "Using twilight type: %s, buffer: %s minutes", twilight_type, buffer_minutes
        )
        ...
        if twilight_type == "civil":
            ...
            self.logger.warning(
                "Unknown twilight type %s, using nautical", twilight_type
            )
            dusk = s["dusk"]
            dawn = s["dawn"]
        ...
        self.logger.debug(
            "Calculated times - Dusk: %s, Dawn: %s (with %s min buffer)",
            dusk_with_buffer,
            dawn_with_buffer,
            buffer_minutes,
        )
    ...
    def _calculate_static_times(
        self, target_date: date, timezone: pytz.BaseTzInfo
        ...
        try:
            ...
            raise ValueError(
                f"Invalid time format in static_times configuration: {e!s}"
            ) from e
    ...
    def get_night_range(
        self, current_date: date | None = None, lookback_nights: int = 0
    ) -> tuple[datetime, datetime]:
        """
        ...
        self.logger.info(
            "Calculating night range for %s (lookback: %s nights)",
            target_date,
            lookback_nights,
        )
        ...
        try:
            ...
            if static_config:
                ...
                raise ValueError(
                    f"Unable to calculate times and no static fallback available: {e!s}"
                ) from e

    def get_multiple_night_ranges(
        self, current_date: date | None = None, nights_count: int | None = None
        ...
        for lookback in range(nights_count):
            try:
                ...
                self.logger.error(
                    "Failed to calculate night range for lookback %s: %s",
                    lookback,
                    str(e),
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate dusk/dawn times for observations"
    )
    parser.add_argument(
        "--config", default="config/template.yaml", help="Configuration file path"
    )
    parser.add_argument(
        "--date", type=str, help="Target date (YYYY-MM-DD format, defaults to today)"
    )
    parser.add_argument(
        "--lookback",
        type=int,
    ...
    parser.add_argument(
        "--nights", type=int, help="Number of consecutive nights to calculate"
    )
    ...
    try:
        ...
        if args.nights:
            ...
            print(f"Night range for {target} (lookback {args.lookback} nights):")
