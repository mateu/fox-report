...
def sample_database():
    ...
    sample_events = [
        ('event_1', 'front_yard', 'fox', 1691000000.0, 1691000300.0, 0.95, 1, 1, '/media/event_1_thumb.jpg'),
        ('event_2', 'back_yard', 'fox', 1691001800.0, 1691002000.0, 0.88, 1, 0, '/media/event_2_thumb.jpg'),
        ('event_3', 'side_yard', 'fox', 1691003600.0, 1691003900.0, 0.92, 0, 1, '/media/event_3_thumb.jpg'),
    ]
...
class TestDatabaseQuery:
    ...
    def test_get_fox_events_basic(self, mock_event_data):
        ...
        with patch('database_query_enhanced._attempt_database_connection') as mock_connect, \
             patch('database_query_enhanced._validate_media_files', return_value=mock_event_data):
            
            # Mock database connection and cursor
            mock_cursor = MagicMock()
            mock_cursor.fetchall.return_value = [
                ('event_1', 'front_yard', 0.95, 1691000000.0, 1691000300.0, 1, '/media/event_1_thumb.jpg')
            ]
    ...
    def test_get_fox_events_database_error(self):
        ...
        with patch('database_query_enhanced._attempt_database_connection', side_effect=DatabaseError("Database error")):
            nights = [0]
            dusk_dawn_ranges = [(datetime(2023, 8, 1, 19, 0), datetime(2023, 8, 2, 7, 0))]
    ...
    def test_get_fox_events_with_timeline_segments_enabled(self, mock_event_data):
        ...
        with patch('database_query_enhanced._attempt_database_connection') as mock_connect, \
             patch('database_query_enhanced._validate_media_files', return_value=mock_events_with_timeline):
    ...
    def test_empty_results(self):
        ...
        with patch('database_query_enhanced._attempt_database_connection') as mock_connect, \
            ...
            results_timeline = get_fox_events_with_timeline_segments(nights, dusk_dawn_ranges)
            assert results_timeline == []
    
    def test_test_database_connection(self):
        """Test the database connection test function."""
        with patch('database_query_enhanced._attempt_database_connection', return_value=MagicMock()):
            result = test_database_connection()
            assert result is True
        
        with patch('database_query_enhanced._attempt_database_connection', side_effect=DatabaseError("No DB")):
            result = test_database_connection()
            assert result is False
    
    @pytest.mark.parametrize("nights_count,expected_calls", [
        (1, 1),
        (3, 3),
        (7, 7)
    ])
    def test_multiple_nights_query_count(self, nights_count, expected_calls, mock_event_data):
