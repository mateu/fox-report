...
def test_get_fox_events():
    ...
    for night in nights:
        ...
        print(
            f"  Night {night}: {dusk.strftime('%Y-%m-%d %H:%M')} to {dawn.strftime('%Y-%m-%d %H:%M')}"
        )
    ...
    if fox_events:
        ...
        for i, event in enumerate(fox_events[:3]):  # Show first 3 events
            print(f"  Event {i + 1}:")
            print("    ID: {}".format(event["event_id"]))
            print("    Camera: {}".format(event["camera"]))
            print("    Confidence: {:.2f}".format(event["confidence"]))
            print("    Duration: {:.1f} minutes".format(event["duration"]))
            print("    Start: {}".format(event["start_time"]))
            print("    Has clip: {}".format(event["clip"]))
            print(f"    Night index: {event['night_index']}")
            print()
    else:
...
def test_get_fox_events_with_timeline():
    ...
    fox_events = get_fox_events_with_timeline_segments(
        nights, dusk_dawn_ranges, include_timeline=True
    )

    print(f"Found {len(fox_events)} fox events with timeline data")

    if fox_events:
        for event in fox_events[:2]:  # Show first 2 events
            print("\nEvent {}:".format(event["event_id"]))
            print(f"  Timeline segments: {len(event.get('timeline_segments', []))}")
